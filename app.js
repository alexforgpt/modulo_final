...
app.post('/users', (req, res) => {
  ...
  if (!first_name || !last_name || !email) {
    return res.status(400).send('Campos obrigatórios: first_name, last_name, email');
  }

  const query = `INSERT INTO ${NOME_TABELA} (first_name, last_name, email) VALUES ("${first_name}", "${last_name}", "${email}")`;
